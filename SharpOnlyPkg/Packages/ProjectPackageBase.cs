...
namespace BoydYang.SharpBuildPkg.Packages
{
    public abstract class ProjectPackageBase : Package
    {
        #region fields
        /// <summary>
        ...
        /// </summary>
        private Dictionary<Guid, string> projectTables = new Dictionary<Guid, string>();
        ...
        public string GetProjectTrustLevel(Guid projectInstance)
        {
            if (!this.projectTables.ContainsKey(projectInstance))
            {
                return string.Empty;
            }

            return projectTables[projectInstance]; ;
        }
        ...
        public void SetProjectTrustLevel(Guid projectInstance, string value)
        {
            ...
            if (this.projectTables.ContainsKey(projectInstance))
            {
                this.projectTables[projectInstance] = value;
            }
            else
            {
                this.projectTables.Add(projectInstance, value);
            }
        }
            ...
            if (string.Compare(key, this.ProjectTrustPersistenceKey, StringComparison.OrdinalIgnoreCase) == 0 && stream != null)
            {
                using (BinaryReader reader = new BinaryReader(stream))
                {
                    ...
                    for (int i = 1; i <= projects; i++)
                    {
                        ...
                        this.projectTables.Add(new Guid(projectGuid), trustlevel);
                    }
                }
            }
            else
        ...
        protected override void OnSaveOptions(string key, Stream stream)
        {
            if (string.Compare(key, this.ProjectTrustPersistenceKey, StringComparison.OrdinalIgnoreCase) == 0 && stream != null)
            {
                using (BinaryWriter writer = new BinaryWriter(stream))
                {
                    ...
                    writer.Write((int)this.projectTables.Count);
                    foreach (Guid projectGuid in this.projectTables.Keys)
                    {
                        writer.Write((string)projectGuid.ToString("B"));
                        writer.Write(this.projectTables[projectGuid].ToString());
                    }
                }
